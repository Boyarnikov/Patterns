...
def cor_handle_log_act(f, log_data):  # обработчик act-ов
    if log_data.type in 'act_info':
        ...
        if log_data.act == "atk":
            ...
            cor_handle_log(f, f.make_unit_info_log(f.find_unit_by_unit_name(log_data.name)))
...
def cor_handle_log_hit(f, log_data):  # обработчик hit-ов
    if log_data.type in 'hit_info':
        ...
        for name in log_data.whom:
            ...
            after_logs = list()
            after_logs.extend(f.interface.hit(unit, log_data.hit))
            new_logs.append(f.make_unit_info_log(unit))
            new_logs.extend(after_logs)
...
def cor_handle_log_status(f, log_data):  # обработчик наложения статусов
    if log_data.type in 'status_info':
        ...
        for index in f.order_atk:
            if f.group_atk.items[index].name in log_data.whom:
                f.group_atk.items[index] = f.interface.set_status(f.group_atk.items[index], log_data.status)
                cor_handle_log(f, f.make_unit_info_log(f.group_atk.items[index]))
        for index in f.order_def:
            if f.group_def.items[index].name in log_data.whom:
                f.group_def.items[index] = f.interface.set_status(f.group_def.items[index], log_data.status)
    else:
        cor_handle_log_death(f, log_data)


def cor_handle_log_death(f, log_data):  # обработчик смерти
...
class Fight:
    ...
    def handle_units_names_find(self, unit_name, whom):  # возвращает список имён по запросу имён через whom = "?..."
        ...
        if whom[1:] == 'self':
            ...
            my_group_names.extend(enemy_group_names)
            return my_group_names
        else:
            return []
        return []
