        ...
        for act in ACT_TYPES:
            if act in set_profs.keys():
                if not 0 <= set_profs[act] <= 1:
                    raise ValueError("Неверное значение професиональности", act, '=', set_profs[act], "передано")
...
class CursedSoldier(Soldier):
    ...
    def __str__(self):
        ...
        info += f'произойдёт событие "{self.curse.deals}" для "{self.curse.whom}" \n'
...
class BlessedSoldier(Soldier):
    ...
    def __str__(self):
        info = Soldier.__str__(self)
        info += f'Этот солдат благословлён, поэтому при событии "{self.bless.type}"\n'
        info += f'произойдёт событие "{self.bless.deals}" для "{self.bless.whom}" \n'
        info += f'с вероятностью {int(self.luck * 100)}%\n'
        return info
