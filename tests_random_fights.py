...
ACT_GENERATOR = ['atk', 'atk', 'atk', 'atk', 'atk', 'atk', 'atk', 'atk', 'atk', 'def', 'def', 'def', 'heal']
tests.run_tests()

A = 0.0
for testses in range(1000):
    units_list = list()
    for index in range(20):
        init = max(0, r.randint(-8, 2))
        if init > 0:
            units_list.append(units.LeaderSoldier(str(index), r.randint(1, 5), r.randint(1, 3), init))
        else:
        ...
        units_list[index].set_atk(r.sample(ACT_GENERATOR, r.randint(1, 10)))
        units_list[index].set_def(r.sample(ACT_GENERATOR, r.randint(1, 10)))
